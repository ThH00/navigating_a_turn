...
% animation
myVideo = VideoWriter('sleigh_angled_0.66pi.mp4', 'MPEG-4');
...
function duxdt = bheom(t,ux)
    ...
    % theta = ux(5);
    global m1 m2 ell beta
    duxdt = [(m1*ux(2)*((sin(2*beta)*ux(1))/2 - ell*cos(beta)^2*ux(2)))/(m2 + m1*cos(beta)^2);
        (2*ux(2)*(m1*ux(1) + m2*ux(1) - (ell*m1*sin(2*beta)*ux(2))/2))/(ell*(2*m2 + 2*m1*cos(beta)^2));
        cos(beta + ux(5))/cos(beta)*ux(1)+ell*sin(ux(5))*ux(2);
        sin(beta + ux(5))/cos(beta)*ux(1)-ell*cos(ux(5))*ux(2);
        ux(2)];
end

    % dydt(1) = cos(beta + ux(5))/cos(beta)*ux(1)+l*sin(ux(5))*ux(2); 
    % dydt(2) = sin(beta + ux(5))/cos(beta)*ux(1)-l*cos(ux(5))*ux(2);
    % dydt(3) = ux(2);

%     dudt(1) = (m1*ux(2)*((sin(2*beta)*ux(1))/2 - l*cos(beta)^2*ux(2)))/(m2 + m1*cos(beta)^2);
%     dudt(2) = (2*ux(2)*(m1*ux(1) + m2*ux(1) - (l*m1*sin(2*beta)*ux(2))/2))/(l*(2*m2 + 2*m1*cos(beta)^2));